...
class ProductsProvider with ChangeNotifier{
  ...
  List<ProductModel> findByCategory(String categoryName){
    return _productsList.where((element) => element.productCategoryName.toLowerCase().contains(categoryName.toLowerCase())).toList();
  ...
  List<ProductModel> searchQuery(String searchText){
    List<ProductModel> searchList =
     _productsList.where((element) => element.title.toLowerCase().contains(searchText.toLowerCase())).toList();
    return searchList;
  }
...
static  List<ProductModel> _productsList = [];
  ...
  Future<void> fetchProducts()async{
    _productsList = [];
   // _productsList.clear();
    await FirebaseFirestore.instance.collection('products').get().then((QuerySnapshot snapshot) {
     for (var element in snapshot.docs) {
       _productsList.insert(0, ProductModel( //The insert method in Flutter is used to add an element at a specific index in a List.
         id: element.get('id'),
